...
class OpAmp:
    ...
    def omega_knee(self, x, bias=1.0, vt=0.001):
        v0 = vt * omega((bias + 1) / vt) - 2
        expx = np.exp(-x / vt)
        in_omega = (-x + bias + expx) / vt
        v_out = vt * omega(in_omega) - expx
        return -(v_out - 1 - v0) / (1 + v0)

    def process_sample_omega(self, x):
        bias = 1.0
        vt = 0.0026
        k = 100
        x1 = bias - k * vt

        # Knee value and derivative at the junction
        v1 = self.omega_knee(x1, bias=bias, vt=vt)
        eps = 1e-4
        v1p = (
            self.omega_knee(x1 + eps, bias=bias, vt=vt)
            - self.omega_knee(x1 - eps, bias=bias, vt=vt)
        ) / (2 * eps)

        # Coefficients for the quadratic central part
        a = (v1p * x1 - v1) / (2 * x1**3)
        b = (-v1p * x1 + 3 * v1) / (2 * x1)

        x_abs = abs(x)
        if x_abs >= x1:
            y = self.omega_knee(x_abs, bias=bias, vt=vt)
        else:
            y = a * x_abs**3 + b * x_abs

        return np.sign(x) * y

    def process_sample_tanh(self, x):
        n = 100
        return x / (1 + x**n) ** (1 / n)
...
if __name__ == "__main__":
    ...
    y = [amp.process_sample_tanh(v) for v in x]
    y2 = [amp.process_sample_omega(v) for v in x]

    dy = np.gradient(y, x)
    dy2 = np.gradient(y2, x)

    # plt.plot(x, dy, color="green", label="dy/dx")
    plt.plot(x, dy2, color="blue", label="omega")
    plt.axhline(1, color="red", linestyle="--", label="unity gain")
    plt.axvline(-1.0 - 0.001, color="black", linestyle="--")
