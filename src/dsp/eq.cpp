...
void EQ::setCoefficients()
{
    ...
    auto low_shelf_coefficients = juce::dsp::IIR::Coefficients<float>::makeLowShelf(
        processSpec.sampleRate, low_shelf_freq, low_shelf_q, current_low_shelf_gain
    );
    *low_shelf_filter.coefficients = *low_shelf_coefficients;
    ...
    auto low_mid_coefficients = juce::dsp::IIR::Coefficients<float>::makePeakFilter(
        processSpec.sampleRate, current_low_mid_freq, current_low_mid_q, current_low_mid_gain
    );
    *low_mid_filter.coefficients = *low_mid_coefficients;
    ...
    auto high_mid_coefficients = juce::dsp::IIR::Coefficients<float>::makePeakFilter(
        processSpec.sampleRate, current_high_mid_freq, current_high_mid_q, current_high_mid_gain
    );
    *high_mid_filter.coefficients = *high_mid_coefficients;

    float current_high_shelf_gain = high_shelf_gain.getCurrentValue();
    auto high_shelf_coefficients = juce::dsp::IIR::Coefficients<float>::makeHighShelf(
        processSpec.sampleRate, high_shelf_freq, high_shelf_q, current_high_shelf_gain
    );
...
{
    if (low_shelf_gain.isSmoothing() ||
        low_mid_freq.isSmoothing() || low_mid_q.isSmoothing() || low_mid_gain.isSmoothing() ||
        high_mid_freq.isSmoothing() || high_mid_q.isSmoothing() || high_mid_gain.isSmoothing() ||
        high_shelf_gain.isSmoothing() || lpf_frequency.isSmoothing())
        setCoefficients();
