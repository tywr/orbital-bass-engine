namespace AmpDimensions
{
constexpr int AMP_WIDTH = 850;
constexpr int AMP_HEIGHT = 400;

constexpr int AMP_FRAME_PADDING = 15;
// constexpr int AMP_FRAME_HEIGHT = 240;

constexpr int AMP_TYPE_BUTTONS_HEIGHT = 100;
constexpr int AMP_TYPE_BUTTON_SIZE = 50;

constexpr int AMP_SIDE_WIDTH = 200;
constexpr int AMP_INNER_TOP_PADDING = 30;
constexpr int AMP_INNER_BOTTOM_PADDING = 25;

constexpr int AMP_BORDER_THICKNESS = 2.0f;
constexpr int AMP_BORDER_RADIUS = 20.0f;

constexpr int AMP_LABEL_HEIGHT = 40;
constexpr int AMP_KNOBS_BOTTOM_BOX_HEIGHT = 80;
constexpr int AMP_SMALL_KNOB_WIDTH = 40;
constexpr int AMP_SMALL_KNOB_HEIGHT = 40;

constexpr int AMP_BYPASS_SIZE = 20;
} // namespace AmpDimensions
