...
void SynthVoiceKnobsComponent::paint(juce::Graphics& g)
{
    juce::ignoreUnused(g);
}

void SynthVoiceKnobsComponent::resized()
{

    auto bounds = getLocalBounds();
    auto label_bounds = bounds.removeFromTop(SynthDimensions::LABEL_HEIGHT);
    const int knob_box_size = bounds.getWidth() / (int)knobs.size();
    for (auto knob : knobs)
    {
        knob.label->setBounds(label_bounds.removeFromLeft(knob_box_size)
                                  .withSizeKeepingCentre(
                                      SynthDimensions::KNOB_SIZE,
                                      SynthDimensions::LABEL_HEIGHT
                                  ));
...
void SynthVoiceKnobsComponent::switchColour(
    juce::Colour colour1, juce::Colour colour2
)
{
    juce::ignoreUnused(colour2);
    for (SynthVoiceKnob knob : knobs)
    {
